...
function ceo_list_jump_to_comic($unhide = false, $exclude = '', $showcount = false, $return = false) {
	ceo_protect();
...
class ceo_comic_list_dropdown_widget extends WP_Widget {
	...
	function ceo_comic_list_dropdown_widget($skip_widget_init = false) {
		if (!$skip_widget_init) {
			$widget_ops = array('classname' => __CLASS__, 'description' => __('Display dropdown list of comics.', 'comiceasel') );
			$this->WP_Widget(__CLASS__, __('Comic Easel - Comic List Dropdown','comiceasel'), $widget_ops);
		}
	}
	...
	function widget($args, $instance) {
		...
		ceo_list_jump_to_comic($instance['unhide'], $instance['exclude'], $instance['showcount']);
		echo $after_widget;
	}
...
add_action( 'widgets_init', create_function('', 'return register_widget("ceo_comic_list_dropdown_widget");') );
